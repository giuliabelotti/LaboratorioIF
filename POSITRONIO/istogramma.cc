...
void istogramma() {
	...
	double mean = 0;
   	...
   	TFile *f = new TFile("wave.root", "RECREATE"); //Creo il file .root
	...
	for (nome = 1; nome <= N ; nome++){
		nuovonome = "wave_out" + to_string(nome) + ".txt";
		...
		*/
		...
		minV = *min_element(Voltage.begin(), Voltage.end()); //trovo il minimo dell'array
		...
		*/
		...
		V_Iter = min_element(Voltage.begin(), Voltage.end())-100;
		for (; V_Iter <= min_element(Voltage.begin(), Voltage.end())+100; V_Iter++)
		{
			y.push_back(*V_Iter);
		}
